...
- **Comments and identifiers in English.** User-facing strings go through i18n (ARB), never
  hard-coded. No Chinese in hand-written code except inside `*.arb` translation files. The
  generated gen_l10n output (`lib/l10n/generated/`) is gitignored and regenerated by `flutter pub
  get` — do NOT commit it, and do NOT hand-escape its CJK (that gets reverted on the next regen).
